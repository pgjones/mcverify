...
CalculateCerenkovYield( const double eStart, // Total energy of the particle at the start [MeV]
                        ...
                        const double nAve = 1.342 ) // for lightwater
{
  ...
  TGraph* dEdx = TheoryStoppingPower();
 
  const double Estep = 0.0001; // Intergral step size
  double N = 0.0;
  range = 0.0;
  ...
  for( double Etotal = me; Etotal <= eStart; Etotal += Estep )
    {
      ...
      const double dN = 370 * z * z * (hc / nMin - hc / nMax) * ( 1.0 - 1.0 / ( nAve * nAve * beta2Func->Eval( Etotal ) ) ) * dx;
      //const double dN = 370 * z * z * ( 3.88939 - 2.15023 / ( beta2Func->Eval( Etotal ) ) ) * dx;
      if( dN > 0.0 )
        N += dN;
    }
  return N;
